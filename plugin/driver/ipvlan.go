...
func (driver *ipvlanDriver) Setup(
	...
	slaveLink, err := netlink.LinkByName(hostVeth)
	if err != nil {
	...
	err = netNS.Do(func(netNS ns.NetNS) error {
		...
		// 2.1 setup addr
		err = netlink.LinkSetName(slaveLink, containerVeth)
		if err != nil {
...
func (driver *ipvlanDriver) Teardown(hostVeth string, containerVeth string, netNS ns.NetNS) error {
	err := netNS.Do(func(netNS ns.NetNS) error {
		...
		if err == nil {
			...
			if err != nil {
				return errors.Wrapf(err, "error del ipvlan link: %v", containerVeth)
			}
		}
		return nil
	})
